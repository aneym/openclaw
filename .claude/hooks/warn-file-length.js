...
- Extract render helpers → ${basename}-render${ext}`
  ...
  if (filePath.includes('/extensions/') || filePath.includes('/telegram/') || filePath.includes('/discord/') || filePath.includes('/slack/') || filePath.includes('/signal/')) {
    return `
...
function checkFile(filePath, lineChange, editSize) {
  ...
  if (lineCount === null) return { status: 'ok' } // New file
  ...
  process.stdin.on('end', () => {
    try {
      ...
      const checkPath = (fp) => {
        ...
        if (result.status === 'block') hasBlock = true
