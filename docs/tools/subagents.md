...
- **Non-blocking spawn:** `sessions_spawn` returns `{ status: "accepted", runId, childSessionKey }` immediately.
- **Scoped context:** Sub-agent sessions include `AGENTS.md`, `TOOLS.md`, and optional `TOOLS.d/**/*.md` shards only.
</Warning>
