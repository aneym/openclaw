...
public final class OpenClawChatViewModel {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenClawChatMessage] {
        ...
        let sanitized = decoded.map(Self.sanitizeUserFacingMessage)
    ...
    private static func dedupeKey(for message: OpenClawChatMessage) -> String? {
        ...
        // Bucket timestamps to reduce accidental duplicates when the same message exists
        // in multiple transcript sources (e.g. pending user message vs flushed transcript).
        let tsBucketSeconds = Int(timestamp / 1000.0)
        return "\(message.role)|\(tsBucketSeconds)|\(text)"
    }
    ...
    private static func sanitizeUserFacingMessage(_ message: OpenClawChatMessage) -> OpenClawChatMessage {
        // Some inbound channels embed prompt-context blocks (conversation/sender/thread info)
        // into the user-role text. Keep those for the agent, but don't show them in UI.
        if message.role.lowercased() != "user" {
            return message
        }
        var changed = false
        let nextContent = message.content.map { block -> OpenClawChatMessageContent in
            let kind = (block.type ?? "text").lowercased()
            guard (kind == "text" || kind.isEmpty), let text = block.text, !text.isEmpty else {
                return block
            }
            let cleaned = Self.sanitizeUserFacingText(text)
            if cleaned == text {
                return block
            }
            changed = true
    ...
    private static let untrustedContextHeaderRE: NSRegularExpression = {
        // Matches a single leading untrusted context block like:
        // "Conversation info (untrusted metadata):\n```json\n...\n```\n"
        // This is generated by src/auto-reply/reply/inbound-meta.ts.
        let pattern = #"(?s)^\s*[A-Za-z][^\n]*\(\s*untrusted[^)]*\)\s*:\s*\n```json\s*\n.*?\n```\s*\n*"#
        return (try? NSRegularExpression(pattern: pattern)) ??
            (try! NSRegularExpression(pattern: "(?!)"))
    }()

    private static let envelopeLineRE: NSRegularExpression = {
        // Matches a line starting with "[...]" envelope prefix.
        let pattern = #"^\[([^\]]+)\]\s*"#
        return (try? NSRegularExpression(pattern: pattern)) ??
            (try! NSRegularExpression(pattern: "(?!)"))
    }()

    private static let messageIdLineRE: NSRegularExpression = {
        let pattern = #"^\s*\[message_id:\s*[^\]]+\]\s*$"#
        return (try? NSRegularExpression(pattern: pattern, options: [.caseInsensitive])) ??
            (try! NSRegularExpression(pattern: "(?!)"))
    }()

    private static let envelopeHeaderDateRE: NSRegularExpression = {
        // Rough match: contains "YYYY-MM-DD HH:MM"
        let pattern = #"\b\d{4}-\d{2}-\d{2}\b.*\b\d{2}:\d{2}\b"#
        return (try? NSRegularExpression(pattern: pattern)) ??
            (try! NSRegularExpression(pattern: "(?!)"))
    }()
    ...
    private static func sanitizeUserFacingText(_ text: String) -> String {
        ...
        // Strip leading untrusted context blocks (conversation/sender/reply/etc).
        while true {
            let range = NSRange(s.startIndex..<s.endIndex, in: s)
            guard let match = self.untrustedContextHeaderRE.firstMatch(in: s, range: range) else { break }
            guard match.range.location == 0, let r = Range(match.range, in: s) else { break }
            s.removeSubrange(r)
        }

        // Drop any standalone [message_id: ...] hint lines.
        let lines = s.split(separator: "\n", omittingEmptySubsequences: false)
        var nextLines: [String] = []
        nextLines.reserveCapacity(lines.count)
        for rawLine in lines {
            let line = String(rawLine)
            let range = NSRange(line.startIndex..<line.endIndex, in: line)
            if self.messageIdLineRE.firstMatch(in: line, range: range) != nil {
                continue
            }
            nextLines.append(line)
        }

        // Strip envelope prefix per-line when it looks like a timestamp/channel envelope.
        for i in nextLines.indices {
            let line = nextLines[i]
            let range = NSRange(line.startIndex..<line.endIndex, in: line)
            guard let match = self.envelopeLineRE.firstMatch(in: line, range: range),
                  match.numberOfRanges >= 2,
                  let headerRange = Range(match.range(at: 1), in: line),
                  let fullRange = Range(match.range(at: 0), in: line)
            else { continue }
            let header = String(line[headerRange])
            let headerNS = header as NSString
            let headerFullRange = NSRange(location: 0, length: headerNS.length)
            guard self.envelopeHeaderDateRE.firstMatch(in: header, range: headerFullRange) != nil else {
                continue
            }
            nextLines[i] = String(line[fullRange.upperBound...])
        }
