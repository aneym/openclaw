...
export function updateBranchRatio(root: SplitNode, branchId: string, ratio: number): SplitNode {
  return mapNode(root, (node) => {
...
export function setLeafThread(root: SplitNode, paneId: string, threadId: string): SplitNode {
  return mapNode(root, (node) => {
...
export function moveLeafBeside(
  ...
  if (!findLeaf(pruned, targetPaneId)) {
    return null;
  }
...
export function swapLeafThreads(root: SplitNode, paneIdA: string, paneIdB: string): SplitNode {
  const leafA = findLeaf(root, paneIdA);
  const leafB = findLeaf(root, paneIdB);
  if (!leafA || !leafB || paneIdA === paneIdB) {
    return root;
  }
...
export function reconcileTreeWithPanes(
  ...
  if (arraysEqual(existingKeys, paneSessionKeys)) {
    return existingTree;
  }
  return buildBalancedTree(paneSessionKeys);
}
