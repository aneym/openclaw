...
function detectPhase(toolName: string, args: Record<string, unknown>): CodingPhase {
  ...
  if (toolName === 'Bash' || toolName === 'exec') {
    ...
    // Test-like commands
    if (/\b(test|spec|jest|pytest|vitest|mocha|npm test|pnpm test|bun test|cargo test|go test)\b/i.test(cmd)) {
      return 'testing'
    }
    // Build commands
    return 'building'
...
export function useCodingSession(sessionKey: string) {
  ...
  // Subscribe to new messages and update events
  useEffect(() => {
    if (!sessionKey) {
      return
    }
    ...
    const unsubscribe = subscribe('session.message', (payload) => {
      ...
      if (event.sessionKey === sessionKey) {
        ...
        setEvents((prevEvents) => {
          const currentPhase = prevEvents.length > 0
            ? prevEvents[prevEvents.length - 1].phase
            : 'exploring'
